let root = document.querySelector(".twoPic .swiper .swiper-wrapper");

async function loadTwoPic() {
  try {
    let data = await fetch(
      "https://raw.githubusercontent.com/shadi-mohaddesi/db-epic/main/db.json"
    );
    let res = await data.json();
    let result = res.twoPic
      .map((item) => {
        return `
    ...
    var swiper = new Swiper(".twoPicSlider", {
      slidesPerView: 1,
      spaceBetween: 10,
      pagination: {
